...
/**
 * 说明：站内信 创建人：FH Q313596790 创建时间：2016-01-17
 */
@Controller
...
public class FhsmsController extends BaseController {
	...
	public ModelAndView list(Page page) throws Exception {
		...
		// 兼容basecontroller 中查找用户类型为2的注册用户
		if (!"2".equals(pd.getString("TYPE_TEMP"))) {
			pd.put("TYPE", 1);
		}
	...
	public ModelAndView goView() throws Exception {
		...
		// 兼容basecontroller 中查找用户类型为2的注册用户
		if ("1".equals(pd.getString("TYPE_TEMP")) && "2".equals(pd.getString("STATUS"))) {
			pd.put("TYPE", 1);
			fhsmsService.edit(pd);
		}
