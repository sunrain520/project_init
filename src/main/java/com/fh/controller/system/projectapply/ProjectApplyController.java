...
public class ProjectApplyController extends BaseController {
	...
	public ModelAndView exportExcel() throws Exception{
		...
		titles.add("申请项目ID");	//4
		titles.add("申请份数");	//5
		titles.add("邮寄联系人姓名");	//6
		titles.add("邮寄联系人电话");	//7
		titles.add("邮寄地址");	//8
		titles.add("招标文件上传");	//9
		titles.add("申请时间");	//10
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var2", varOList.get(i).getString("FILE_TYPE"));	    //2
			vpd.put("var3", varOList.get(i).getString("PROJECT_NAME"));	    //3
			vpd.put("var4", varOList.get(i).getString("PROJECT_ID"));	    //4
			vpd.put("var5", varOList.get(i).get("NUM").toString());	//5
			vpd.put("var6", varOList.get(i).getString("ADDRESSEE"));	    //6
			vpd.put("var7", varOList.get(i).getString("PHONE"));	    //7
			vpd.put("var8", varOList.get(i).getString("ADDR"));	    //8
			vpd.put("var9", varOList.get(i).getString("FILE_LIST"));	    //9
